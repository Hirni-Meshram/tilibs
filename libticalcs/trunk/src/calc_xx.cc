...
TIEXPORT3 int TICALL ticalcs_calc_send_lab_equipment_data(CalcHandle *handle, CalcModel model, CalcLabEquipmentData * lab_equipment_data)
{
	...
	handle->busy = 1;
	if (calc->send_lab_equipment_data)
	{
		CalcEventData event;
		...
		if (!ret)
		{
			ret = calc->send_lab_equipment_data(handle, model, lab_equipment_data);
		}
...
TIEXPORT3 int TICALL ticalcs_calc_get_lab_equipment_data(CalcHandle *handle, CalcModel model, CalcLabEquipmentData * lab_equipment_data)
{
	...
	handle->busy = 1;
	if (calc->get_lab_equipment_data)
	{
		CalcEventData event;
		...
		if (!ret)
		{
			ret = calc->get_lab_equipment_data(handle, model, lab_equipment_data);
		}
